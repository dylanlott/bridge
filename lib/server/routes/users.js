...
UsersRouter.prototype.softDeleteUser = function(req, res, next) {
  const User = this.storage.models.User;
  const PaymentProc = this.storage.models.PaymentProcessor;
  ...
  User.findOne({ _id: req.params.id }, function(err, user) {
    ...
    if (!user) {
      return next(new errors.NotFoundError('User not found', req.params.id));
    }

    log.info('deleting user', req.params.id);

    PaymentProc.find({ user: req.params.id }) 
      .remove()
      .exec(() => {
        log.info('payment processors deleted'); 
      })
      .then(() => {
        user.deleted = true; 
