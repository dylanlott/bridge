 ...
 * @description Update `user.preferences` such as dnt. Takes a user obejct
 * with preferences to update and sends an email to user notifying them
 * that their preferences were changed.
 */
UsersRouter.prototype.updateUser = function (req, res) {
  ...
  if (req.user === req.params.id) {
    User.findById(req.params.id, function(err, user) {
      user.preferences.dnt = req.body.user.preferences.dnt;
      user.save(function(err) {
        if (err) {
          return next(new errors.InternalError(err.message));
        }

        log.info('settings updated for user %j', user);
        ...
        self.mailer.dispatch(user._id, 'settings-updated', {
          user: user._id,
          updated: user.preferences.toObject(),
          time: Date.now()
        }, function(err) {
          if(err) {
            log.error('failed to send settings-updated email', err.message);
            return next(new errors.InternalError(err.message));
          }
        });
  ...
  res.status(401).send(errors.BadRequestError('Unauthorized'))
}
...
UsersRouter.prototype._definitions = function() {
  return [
    ...
    ['PUT', '/users/:id', this._limiter(defaults), this._verify, rawbody, this.updateUser]
  ];
};
